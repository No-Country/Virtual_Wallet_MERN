...
const CartPresupuesto = ({ objeto, onDelete }) => {
  const porcentaje = `w-[${(objeto.monto * 100 / objeto.total).toFixed(0)}%]`
  ...
  return (
      ...
      <div className='head w-full h-auto flex flex-row flex-nowrap justify-start items-center p-1 gap-1'>
        <div className="w-[60%] h-auto flex flex-row flex-nowrap justify-start items-center p-1 gap-1">
          ...
          <div className={`icono w-[30px] h-[30px] rounded-[50%] ${objeto.color} flex justify-center items-center text-[#ffff] text-[1.3rem]`}>
        ...
        <div className='porcentajes w-[40%] h-auto flex flex-row flex-nowrap justify-start items-center p-1 gap-1'>
          <p className='w-full font-parrafo font-[400] text-bgSubmenu text-[.7rem] flex flex-row flex-nowrap justify-end items-end gap-1'>
            ...
            <span className='font-[700] text-hoverBotonSubmenu'>s/{objeto.total}</span>
          </p>
        </div>
      </div>
      <div className='barraProgreso w-full h-[10px] bg-fondo rounded-[10px] relative'>
        <div className={`progreso h-[10px] ${objeto.color} rounded-[10px] ${porcentaje} transition-all duration-300`}></div>
      </div>
      <button
      onClick={handleDelete} 
      className={`absolute top-1 right-1 w-[25px] h-[25px] rounded-[4px] ${objeto.color} flex justify-center items-center text-colorFuente2Submenu text-[1rem] hover:bg-hoverBotonSubmenu transition-all duration-300 ease-in-out`}>
